// src/pages/Editais/App.jsx
import React, { useState, useEffect } from 'react';

export default function Editais({ categoriaInicial }) {
  const [filtroAtivo, setFiltroAtivo] = useState(categoriaInicial || 'Todos');
  const [editalExpandidoId, setEditalExpandidoId] = useState(null); // Controla qual edital está aberto

  useEffect(() => {
    setFiltroAtivo(categoriaInicial);
    setEditalExpandidoId(null); // Fecha o edital se o filtro mudar
  }, [categoriaInicial]);

  // Mock de dados enriquecido com os "Resumos"
  const todosEditais = [
    { 
  ...
  const alternarEdital = (id) => {
    if (editalExpandidoId === id) {
      setEditalExpandidoId(null); // Fecha se clicar de novo
    } else {
      setEditalExpandidoId(id); // Abre o selecionado
    }
  };
        ...
        {editaisFiltrados.length > 0 ? (
          editaisFiltrados.map(edital => (
            <div key={edital.id} className={`bg-white/5 border transition-all rounded-2xl overflow-hidden ${editalExpandidoId === edital.id ? 'border-[#3ab0ff] bg-white/10' : 'border-white/10 hover:border-[#3ab0ff]/50 hover:bg-white/10'}`}>
              ...
              {/* Acordeão: Resumo do Edital (Só aparece se clicar em Detalhes) */}
